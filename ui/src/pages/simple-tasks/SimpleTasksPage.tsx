...
function SimpleTasksPage() {
	...
	const simpleTasks = useMemo(() => {
		return allTasks.filter((task) => task.tags.some((tag) => tag.name.toLowerCase() === 'quick-task'))
	}, [allTasks])

	const stats = useMemo(() => {
		const completed = simpleTasks.filter(
			(task) => task.status.status.toLowerCase() === 'complete' || task.status.status.toLowerCase() === 'closed',
		).length
		...
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-sm text-slate-500 mt-1">Simple tasks without hierarchy - get things done fast!</p>
				</div>
				...
				<Card className="p-6 bg-gradient-to-br from-emerald-50 to-green-50 border-emerald-200">
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-3xl font-bold text-emerald-900 mt-2">{stats.total}</p>
						</div>
				...
				{simpleTasks.length > 0 ? (
					<div className="space-y-2">
						{simpleTasks.map((task) => {
							const isCompleted = task.status.status.toLowerCase() === 'complete' || task.status.status.toLowerCase() === 'closed'
							...
							return (
								...
								>
									...
									<div className="flex-1 min-w-0">
										...
										{task.text_content && <p className="text-sm text-slate-500 mt-1 line-clamp-1">{task.text_content}</p>}
									</div>
					...
					<div className="text-center py-16">
						...
						<h3 className="text-lg font-semibold text-slate-900 mb-2">No quick tasks yet</h3>
			...
			<CreateTaskModal open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen} defaultType="simple" />
		</div>
	)
}
