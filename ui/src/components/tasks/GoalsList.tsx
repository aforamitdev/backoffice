			...
			<Card className="overflow-hidden border-slate-200  duration-200 mb-0 gap-2">
				...
				{isOpen && (
					<div className="bg-white animate-in slide-in-from-top-2 duration-200">
						...
						{Boolean(goal.children.length) && <MilestonesList milestones={goal.children} />}
					</div>
				)}
			</Card>

			<CreateTaskModal open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen} defaultType="milestone" parentTask={goal} />
		</>
	)
}
