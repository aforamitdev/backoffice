...
import { Target, Milestone as MilestoneIcon, CheckSquare, ChevronRight, X, Loader2 } from 'lucide-react'
...
function CreateTaskModal({ open, onOpenChange, defaultType = 'goal', parentTask }: CreateTaskModalProps) {
	const [selectedType, setSelectedType] = useState<TaskType>(defaultType)
	const [taskName, setTaskName] = useState('')
	const [description, setDescription] = useState('')
	const [selectedParent, setSelectedParent] = useState<Task | null>(parentTask || null)
	...
	const availableParents = useMemo(() => {
		const getValidParents = (type: TaskType): Task[] => {
			switch (type) {
				...
				case 'subtask':
					return allTasks.flatMap((goal) => goal.children.flatMap((milestone) => milestone.children))
				default:
					return []
			}
		}
				...
				<form onSubmit={handleSubmit}>
					<div className="space-y-6 py-4">
						...
						{!isSimpleMode && selectedType !== 'goal' && (
							<div className="space-y-3">
								<Label>
									Parent {getParentTypeLabel(selectedType)} <span className="text-slate-500">(Optional)</span>
								</Label>
								{selectedParent ? (
									<div className="flex items-center gap-2 p-3 bg-slate-50 rounded-lg border border-slate-200">
										...
										<span className="flex-1 text-sm font-medium truncate">{selectedParent.name}</span>
										...
										{availableParents.length > 0 ? (
											availableParents.map((parent) => (
												...
												>
													...
													<p className="text-xs text-slate-500 truncate">{parent.text_content || 'No description'}</p>
												</button>
											))
										) : (
						...
						{!isSimpleMode && selectedParent && (
							<div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl">
								<p className="text-xs font-bold text-blue-900 mb-2 uppercase tracking-wide">📍 Hierarchy Preview</p>
								<div className="flex items-center gap-2 text-sm">
									<span className="text-slate-700 font-medium">{selectedParent.name}</span>
									<ChevronRight className="h-4 w-4 text-slate-400" />
									<span className="font-bold text-slate-900">{taskName || `New ${taskTypeConfig[selectedType].label}`}</span>
								</div>
							</div>
						)}
						...
						{isSimpleMode && (
							<div className="p-4 bg-gradient-to-r from-emerald-50 to-green-50 border-2 border-emerald-200 rounded-xl">
								<p className="text-xs font-bold text-emerald-900 mb-1 uppercase tracking-wide">⚡ Quick Task</p>
								<p className="text-xs text-emerald-700">This task will be created as a standalone item. You can organize it later!</p>
							</div>
						)}
					</div>
